...
function _drupalgap_form_submit(form_id) {
  try {
    ...
    // UPDATE - this is probably fixed now that we are using local storage to
    // retrieve the assembled form.
    ...
    if (local_storage_form) {
      ...
      alert('_drupalgap_form_submit - click - failed to load form from local storage: ' + form_id);
    }
